...
#    nr_k = a * (k3 * np.exp(-fine_time / tau1) + k4)  # New approach 3 only
    ...
    gen_current = gen_current + ia_current  # new approach 2 & 4 only
...
if __name__ == '__main__':
    rough_time = np.genfromtxt('./Shawn model data/dcon_disp3_time.csv',
                               delimiter=',')
    rough_stress = np.genfromtxt('./Shawn model data/'
                                 'dcon_disp3_stress.csv', delimiter=',')
...
#    gen_current = stress_to_current(fine_time, fine_stress, tau1_m, tau2_ap2, k1_ap2, 'gen')
    # New approach 3: add k3 and k4
#    gen_current = stress_to_current(fine_time, fine_stress, tau1_ap3, tau2_ap3, k1_ap3, 'gen')
    # New approach 4: add tau3 and c, don't change other parameters
    gen_current = stress_to_current(fine_time, fine_stress, tau1_m, tau2_m, k1_m, 'gen')
    nr_current = stress_to_current(fine_time, fine_stress, tau1_m, tau2_m, k1_m, 'nr')
    mc_current = stress_to_current(fine_time, fine_stress, tau1_m, tau2_m, k1_m, 'mc')
    ia_current = stress_to_current(fine_time, fine_stress, tau1_m, tau2_m, k1_m, 'ia')
    # %% Save files
#    np.savetxt("ap4_disp3_fine_time.csv", fine_time, delimiter=",")
#    np.savetxt("ap4_disp3_gen_current.csv", gen_current, delimiter=",")
#    np.savetxt("ap4_disp3_nr_current.csv", nr_current, delimiter=",")
#    np.savetxt("ap4_disp3_mc_current.csv", mc_current, delimiter=",")
#    np.savetxt("ap4_disp3_ia_current.csv", ia_current, delimiter=",")
...
#    np.savetxt("disp2_k1l_gen_current.csv", gen_current_k1l, delimiter=",")
    ...
    axs2.plot(fine_time, -nr_current*1e9, color='0', linestyle='--')
    axs2.plot(fine_time, -mc_current*1e9, color='0', linestyle='-')
    axs2.plot(fine_time, -ia_current*1e9, color='0', linestyle='-.')
